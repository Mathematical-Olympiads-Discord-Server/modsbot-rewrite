...
class Mathjams(Cog):
    def __init__(self, bot: commands.Bot):
        ...
        schedule.every().friday.at("11:55").do(self.schedule_ping, timeslot="a").tag(
            "cogs.mathjams"
        )
        schedule.every().friday.at("19:55").do(self.schedule_ping, timeslot="b").tag(
            "cogs.mathjams"
        )
        schedule.every().saturday.at("02:55").do(self.schedule_ping, timeslot="c").tag(
            "cogs.mathjams"
        )
    ...
    async def ping_mathjams(self, timeslot):
        if self.ping:
            ...
            mathjams_channel = await self.bot.fetch_channel(
                cfg.Config.config["mathjams_channel"]
            )
